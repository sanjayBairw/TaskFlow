...
class AIService {
    static extractSearchQuery(prompt) {
        return prompt
     ...
     */
    static generateFallbackResponse(prompt, now) {
                ...
                tasks: [
                    {
                        ...
                        priority: isHighPriority ? Task_1.TaskPriority.HIGH : Task_1.TaskPriority.MEDIUM,
                        status: Task_1.TaskStatus.PENDING,
                        category: Task_1.TaskCategory.STUDY,
                        tags: ['DSA', 'Study'],
        ...
        if (/breakdown|break down|subtasks|decompose/i.test(prompt)) {
            ...
            const tasks = subtasksData.map((st, idx) => {
                ...
                return {
                    ...
                    priority: Task_1.TaskPriority.HIGH,
                    status: Task_1.TaskStatus.PENDING,
                    category: Task_1.TaskCategory.WORK,
        ...
        return {
            ...
            message: 'To prepare for an App Development Placement, focus on these 5 core areas:\n\n' +
    ...
    static async processCommand(payload) {
        ...
        if (payload.conversationId && payload.userId) {
            try {
                existingConversation = await AIConversation_1.AIConversation.findOne({
        ...
        if (isSearchRequired) {
            try {
                ...
                const rawResults = await searchService_1.SearchService.searchWeb(searchQuery, {
                    count: 8,
...
`;
        try {
            const ai = (0, gemini_1.getGeminiClient)();
            const model = (0, gemini_1.getGeminiModelName)();
            ...
            const validatedTasks = aiValidator_1.AIValidator.validateTaskList(parsed.tasks || [], now);
            const responseData = {
            ...
            if (payload.userId) {
                try {
                    ...
                    if (!conv) {
                        conv = new AIConversation_1.AIConversation({
                            userId: payload.userId,
