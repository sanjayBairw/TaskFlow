...
export type RescheduleMode = '12 HOURS' | '24 HOURS' | 'NEXT DAY' | 'AI RECOMMENDED';

export class RescheduleService {
  public static async rescheduleOverdueTasks(
    userId: string | Types.ObjectId,
    mode: RescheduleMode = '12 HOURS',
    maxRetries: number = 10
  ): Promise<{ rescheduledCount: number; tasks: ITask[] }> {
    const now = new Date();
